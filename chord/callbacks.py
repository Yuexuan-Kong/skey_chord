...
from chord.utils.parallel import is_main_process
...
def add_audio_tensorboard(
    ...
    for i in range(10):
        writer.add_audio(
            "{}/{}/{}/vocals.wav".format(type_set, epoch, i),
            batch[i, :, 0:1],
            global_step=epoch,
            sample_rate=sr,
        )
        writer.add_audio(
            "{}/{}/{}/accompaniment.wav".format(type_set, epoch, i),
            batch[i, :, 1:2],
            global_step=epoch,
            sample_rate=sr,
        )
...
class NaNLoopCallback:
    ...
    def __call__(
        ...
        # WARNING: It blocks the gpus after the process is over
