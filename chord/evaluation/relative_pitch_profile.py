...
import torch.nn
import torchaudio
from chord.model.chromanet import ChromaNet, ChromaNetSeq2Seq
...
class RelativePitchProfile(IterableDataset):  # type: ignore
    def __init__(
        ...
        self.tf_dataloader = (
                ...
                .map(lambda x: (load_audio(x, sr, source,mono, do_norm), x["song_path"]))
...
def get_embedding(
    ...
    with torch.no_grad():
        after_crop = crop_fn(hcqt(new_batch), torch.zeros(len(new_batch, ))).squeeze()
        try:
            yh = chromanet(after_crop)
            yh = rearrange(yh, "b f t -> (b t) f").argmax(axis=1)
...
def main(ckpt_path: str, audio_path: str, extension: str, average:bool) -> None:
    device = set_cpu()
    ckpt = load_checkpoint(ckpt_path, "", {})
    dur, sr = [ckpt["audio"]["dur"], ckpt["audio"]["sr"]]
    num_frames = dur*sr
    model_name = "-".join(ckpt_path.split("/")[-5:-1])
    hcqt = HarmonicVQT().to(device)
    chromanet = ChromaNetSeq2Seq().to(device)
    hcqt_state_dict = {
        k.replace("hcqt.", ""): v
...
def input_args() -> None:
    ...
    parser.add_argument(
        "-a", "--average", default=False, action="store_true", help="if calculating the average over time axis or not"
    )
    parser.add_argument(
