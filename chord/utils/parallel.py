...
class FullGatherLayer(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx: Any, x: torch.Tensor) -> Tuple[torch.Tensor, ...]:  # type: ignore
        output = [torch.zeros_like(x) for _ in range(dist.get_world_size())]
        dist.all_gather(output, x)
        return tuple(output)

    @staticmethod
    def backward(ctx: Any, grads: Union[Tuple[torch.Tensor, ...], List[torch.Tensor]]) -> Any:  # type: ignore
...
def find_free_gpus_manually() -> List[int]:
    ...
    memory_available = [
        True if len(x.split()) > 1 else False
        for x in open("tmp_free_gpu", "r").readlines()
    ]
...
def set_gpus(max_memory: float = 0.5) -> str:
...
def fix_random_seeds(seed: int = 31) -> None:
    """
    Fix random seeds. TODO: this doesn't work for random
    """
...
def model_to_distributed(model: Any, device: int) -> Any:
    ...
    model = nn.parallel.DistributedDataParallel(model, device_ids=[device])  # type: ignore
    return model
